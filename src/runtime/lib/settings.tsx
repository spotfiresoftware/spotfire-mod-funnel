...
import { ModProperty } from "./spotfire";
...
interface RadioInput extends ComponentBase {
    type: "radio";
    values: { label: string; value: string }[];
}

type Components = TextInput | CheckboxInput | RangeInput | RadioInput;

function Settings(props: { children: ComponentChildren; style: SettingsStyle }) {
    const [isOpen, setIsOpen] = useState(false);

    let className = "settings-area " + (isOpen ? "" : "tucked-away");
    return (
        <div>
            <div className={className} style={{ background: props.style.bg, color: props.style.fontColor }}>
                {props.children}
            </div>
...
export function renderSettings(components: Components[], style: SettingsStyle) {
                        ...
                        return (
                            <label>
                                <input
                                    ...
                                    type="radio"
                                    name={c.label}
                                    checked={c.property.value<string>() == v.value}
                                    onChange={(e: any) => {
                                        c.property.set(v.value!);
                                    }}
                                ></input>
                                {v.label}
                            </label>
    ...
    render(<Settings style={style}>{c}</Settings>, document.querySelector("#settings-area")!);
}
